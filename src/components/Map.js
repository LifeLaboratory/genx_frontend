...
function Map(props) {
    ...
    const renderNodeColor = (nodeId) => {
        switch (state) {
            case MapStateEnum.PINGING:
            case MapStateEnum.CREATING_TASK:
                return "node node__default node-controller"
            case MapStateEnum.PULLING_NODE_LIST:
            default:
                return `node ${nodeId !== "4" ? "node__green" : "node__red"} node-controller`
        }
    }
    ...
    return (
        <div className="map">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" height="80vh" width="100%" viewBox="0 0 100 100"
                 className="svg">
